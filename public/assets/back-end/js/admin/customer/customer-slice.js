'use strict' ;
$('#customer-slice').on('submit', function (event){
    event.preventDefault();
    let getData = $('#get-customer-slice-data');
    $.ajaxSetup({
        headers: {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#customer-slice').attr('action',getData.data('action'));
            fetchCustomerSlice();
        },
...
$('.customer-slice-status-form').on('submit', function(event){
    event.preventDefault();
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
...
$(document).on('click','.edit' ,function () {
    ...
    $('#customer-slice').attr('action', update.data('action'));
    let id = $(this).attr('id');
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
            ...
            if (data.length !== 0) {
                ...
                for (let count = 0; count < data.length; count++) {
                    ...
                    html += `<td class="column_name" data-column_name="status" data-id="${data[count].id}">
                                <form action="${getToggleStatusText.data('action')}" method="post" id="customer-slice-status${data[count].id}-form" class="customer-slice-status-form">
                                <input type="hidden" name="id" value="${data[count].id}">
                                    <label class="switcher mx-auto">
                                        <input type="checkbox" class="switcher_input toggle-switch-message" id="customer-slice-status${data[count].id}" name="status" value="1" ${data[count].active_status === 1 ? "checked" : ""}
                    ...
                    html += '<td><a type="button" class="btn btn-outline--danger btn-xs delete square-btn" id="' + data[count].id + '"><i class="tio-delete"></i></a> </td></tr>';
                }
                $('tbody').html(html);
