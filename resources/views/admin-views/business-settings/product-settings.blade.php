...
@section('content')
    <div class="content container-fluid">
        ...
        <form action="{{ route('admin.product-settings.index') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="card">
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 text-capitalize d-flex gap-2 text-capitalize">
                        <img width="20" src="{{dynamicAsset(path: 'public/assets/back-end/img/market_deficiencies.png')}}" alt="">
                        {{translate('market_deficiencies')}}
                    </h5>
                </div>
                <div class="card-body">
                    <div class="row align-items-end">
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <div class="d-flex justify-content-between align-items-center gap-10 form-control">
                                    <span class="title-color text-capitalize">
                                        {{translate('market_deficiencies_status')}}
                                        <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                              data-placement="right"
                                              title="{{translate('if_enabled_market_deficiencies_is_work_automatic') }}">
                                            <img width="16"
                                                 src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}" alt="">
                                        </span>
                                    </span>
{{--                                    @dd($deficiencies)--}}
                                    <label class="switcher" for="market_deficiencies_status">
                                        <input type="checkbox" class="switcher_input toggle-switch-message" value="1" name="market_deficiencies_status"
                                               id="market_deficiencies_status" {{ $deficiencies && $deficiencies->status ? 'checked':'' }}
                                               data-modal-id = "toggle-modal"
                                               data-toggle-id = "market_deficiencies_status"
                                               data-on-image = "digital-product-on.png"
                                               data-off-image = "digital-product-off.png"
                                               data-on-title = "{{translate('want_to_Turn_ON_market_deficiencies').'?'}}"
                                               data-off-title = "{{translate('want_to_Turn_OFF_market_deficiencies').'?'}}"
                                               data-on-message = "<p>{{translate('if_enabled_market_deficiencies_is_work_automatic')}}</p>"
                                               data-off-message = "<p>{{translate('if_disabled_market_deficiencies_is_stop_automatic')}}</p>">
                                        <span class="switcher_control"></span>
                                    </label>
                                </div>
                            </div>
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <div class="d-flex gap-2">
                                    <label class="title-color d-flex text-capitalize"
                                           for="available_percentage">{{translate('available_percentage')}}( {{translate('ex').':'. '0.00'}})</label>
                                    <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                          data-placement="right" title="{{translate('The_percentage_available_to_the_client_to_benefit_from_market_shortcomings_from_his_purchases_of_all_products')}}">
                                        <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}"
                                             alt="">
                                    </span>
                                </div>
                                <input class="form-control" type="text" name="available_percentage" id="available_percentage"
                                       value="{{ $deficiencies->available_percentage ??'' }}"
                                       placeholder="{{translate('ex').':'. '0.00'}}" min="0"
                                >
                            </div>
                        </div>
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <div class="d-flex gap-2">
                                    <label class="title-color d-flex text-capitalize"
                                           for="available_quantity">{{translate('available_quantity')}}</label>
                                    <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                          data-placement="right" title="{{translate('The_quantity_available_to_the_client_to_benefit_from_market_shortcomings')}}">
                                        <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}"
                                             alt="">
                                    </span>
                                </div>
                                <input class="form-control" type="text" name="available_quantity" id="available_quantity"
                                       value="{{ $deficiencies->available_quantity ??'' }}"
                                       placeholder="{{translate('ex').':'.'3'}}">
                            </div>
                        </div>
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <label for="parent_items" class="title-color"><?php echo e(translate('parent_item')); ?></label>
                                <select class="form-control js-select2-custom" name="parent_items">
                                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($category->id); ?>" <?php echo e($deficiencies->parent_items == $category->id?'selected':''); ?> >
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <label for="deficiencies_items" class="title-color"><?php echo e(translate('market_deficiencies_item')); ?></label>
                                <select class="form-control js-select2-custom" name="deficiencies_items">
                                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($category->id); ?>" <?php echo e($deficiencies->deficiencies_items == $category->id?'selected':''); ?> >
                                            <?php echo e($category->name); ?>
                                    </option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
