...
use App\Contracts\Repositories\{BusinessSettingRepositoryInterface,CategoryRepositoryInterface,CurrencyRepositoryInterface};
use App\Enums\ViewPaths\Admin\BusinessSettings;
use App\Http\Controllers\BaseController;
use App\Http\Requests\Admin\{BusinessSettingRequest,LoginSetupRequest,AnalyticsRequest};
use App\Services\BusinessSettingService;
use App\Traits\{FileManagerTrait,SettingsTrait};
...
class BusinessSettingsController extends BaseController
{
    ...
    public function getProductSettingsView(): View
    {
        ...
        $deficiencies = json_decode($this->businessSettingRepo->getFirstWhere(params: ['type'=>'market_deficiencies'])->value ?? '');
        $categories = $this->categoryRepo->getList( dataLimit: 'all');
        return view(BusinessSettings::PRODUCT_SETTINGS[VIEW], compact('digitalProduct', 'brand', 'stockLimit', 'deficiencies','categories'));
    }
    ...
    public function updateProductSettings(Request $request)
    {
        ...
        $this->businessSettingRepo->updateOrInsert(
            type: 'market_deficiencies',
            value: json_encode([
                'status' => $request->get('market_deficiencies_status', 0),
                'available_percentage' => $request->get('available_percentage', 0),
                'available_quantity' => $request->get('available_quantity', 0),
                'parent_items' => $request->get('parent_items', 0),
                'deficiencies_items' => $request->get('deficiencies_items', 0),
            ],0)
        );
        Toastr::success(translate('updated_successfully'));
        return back();
