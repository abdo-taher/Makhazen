...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        foreach (CartManager::get_cart(groupId: $data['cart_group_id'], type: 'checked') as $cartSingleItem) {
            ...
            $market_deficiencies = get_business_settings('market_deficiencies');

            if ($product->sub_category_id == $market_deficiencies['parent_items']){
                $customerBalance +=  ($price * $cartSingleItem['quantity']);
            }elseif ($product->sub_category_id == $market_deficiencies['deficiencies_items']){
                $customerBalance -=  ($price * $cartSingleItem['quantity']);
            }
        }
//        dd($customerBalance);
        if (isset($customerBalance)){
            $userBalance = $user->customerBalance();
            if ($userBalance->exists()){
                $customerBalance > 0 ? $userBalance->update(['balance'=> \DB::raw("balance - $customerBalance")]) : $userBalance->update(['balance'=> \DB::raw("balance + $customerBalance")]);
            }else{
                $userBalance->craete(['balance'=>$customerBalance]);
            }
        }
    ...
    public static function customer_balance($orderItems)
    {
        foreach ($orderItems as $item){
            $market_deficiencies = get_business_settings('market_deficiencies');
            if (in_array($orderItems['sub_category_id'],get_business_settings('market_deficiencies')['item_parent'])){
                $subTotal = '';
            }
