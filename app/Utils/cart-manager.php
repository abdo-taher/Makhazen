...
class CartManager
{
    public static function cart_to_db($request=null)
    {
        ...
        if (session()->has('guest_id') || $request->guest_id) {
            ...
            foreach ($carts as $cart) {
                ...
                $market_def = get_business_settings('market_deficiencies');
                $res = '';
                if ($user != 'offline' && ($cart['product_type'] == 'physical' && $product->subCategory()->value('id') == $market_def['deficiencies_items'])) {
                    $tax = Helpers::tax_calculation(product: $product, price: $cart->price, tax: $cart->tax, tax_type: 'percent');
                    $getProductDiscount = Helpers::get_product_discount($product, $cart->price);
                    $calculateTax = $cart->tax_model == 'include' ? 0 : ($tax * $cart->quantity);
                    $productTotalPrice = ($cart->price * $cart->quantity) + $calculateTax - ($getProductDiscount * $cart->quantity);
                    $res = self::check_market_deficiencies(user:$user,cartItem:$productTotalPrice,quantity:$cart->quantity,market_def:$market_def) == 'hasError';
                }
                if (!$res){
    ...
    public static function add_to_cart($request, $from_api = false): array
    {
        ...
        if (($product['product_type'] == 'physical' && $product->subCategory()->value('id') == $market_def['deficiencies_items']) && $user != 'offline') {
             self::check_market_deficiencies(user:$user,cartItem:$productTotalPrice,quantity:$request['quantity'],market_def:$market_def,errorThrow: true);
        }
        //generate group id
        if ($user == 'offline') {
    ...
    public static function update_cart_qty($request): array
    {
        ...
        if (
            ...
            $cart->product()->first()->subCategory()->value('id') == $market_def['deficiencies_items'] &&
            $user != 'offline'
        )
        {
            self::check_market_deficiencies(user: $user, quantity: $request['quantity'], market_def: $market_def, cartId: $cart->id ,errorThrow: true);
        }
    ...
    public static function check_market_deficiencies($user = null ,$cartItem = null ,$quantity = null ,$market_def = null,$cartId = null ,$errorThrow = null)
    {
        if ($cartItem == null){
            ...
            $getProductDiscount = Helpers::get_product_discount($product, $cart->price);
            $calculateTax = $cart->tax_model == 'include' ? 0 : ($tax * $cart->quantity);
            $cartItem = ($cart->price * $cart->quantity) + $calculateTax - ($getProductDiscount * $cart->quantity);
        }
        $userBalance =  $user->customerBalance()?->value('balance')??0 ;
        $cart = Cart::where([['customer_id',$user->id],['product_type','physical']])->whereHas('product',fn($query)=>$query->where('sub_category_id',$market_def['deficiencies_items']));
        $productTotalPrice = self::cart_total_applied_discount($cart->get());
        $productTotalPrice+= $cartItem;
        if ($userBalance < $productTotalPrice){
           return $errorThrow ? throw new ErrorException(translate('don\'t_have_cash_back_to_buy_market_shortcomings!'),200) : 'hasError';
        }
        $available_quantity = $market_def['available_quantity'];
        $available_quantity_cart = $cart->sum('quantity');
        if (isset($available_quantity) && $available_quantity_cart != null && $available_quantity < $available_quantity_cart+$quantity){
           return $errorThrow ?  throw new ErrorException(translate('you have max number for available quantity!'),200) : 'hasError';
        }
